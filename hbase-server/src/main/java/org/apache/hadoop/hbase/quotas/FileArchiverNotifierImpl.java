...
public class FileArchiverNotifierImpl implements FileArchiverNotifier {
  ...
  private final ConcurrentHashMap<String,Long> recentlyArchivedFiles;
  private final ReadLock readLock;
  private final WriteLock writeLock;
  private volatile long lastFullCompute = -1;
  ...
  public FileArchiverNotifierImpl(Connection conn, Configuration conf, FileSystem fs, TableName tn) {
    ...
    this.fs = fs;
    this.tn = tn;
    recentlyArchivedFiles = new ConcurrentHashMap<>();
  ...
  @Override
  public void addArchivedFiles(Map<String, Long> fileSizes) throws IOException {
    ...
    try {
      ...
      if (!currentSnapshots.isEmpty() && !fileSizes.isEmpty()) {
        Map<String,Long> unreferencedBySnapshotFiles = groupArchivedFiledBySnapshotAndRecordSize(
            currentSnapshots, fileSizes);

        // Typical case: some files were not referenced by a snapshot. Remove all of the files
        // which were not referenced by a snapshot (we don't care about that set). We want to get
        // the set of files which were referenced by a snapshot.
        Map<String,Long> referencedBySnapshotFiles = fileSizes;
        if (!unreferencedBySnapshotFiles.isEmpty()) {
          referencedBySnapshotFiles = new HashMap<>(fileSizes);
          referencedBySnapshotFiles.keySet().removeAll(unreferencedBySnapshotFiles.keySet());
        }

        if (LOG.isTraceEnabled()) {
          LOG.trace(
              "Found files referenced by at least one snapshot: " + referencedBySnapshotFiles);
        }
        // Record the files which a snapshot referenced
        recentlyArchivedFiles.putAll(referencedBySnapshotFiles);
      }
    } finally {
      readLock.unlock();
    }
   ...
   *
   * @returns A Map of files and their sizes which none of the {@code snapshots} referred to.
   */
  Map<String,Long> groupArchivedFiledBySnapshotAndRecordSize(
      List<String> snapshots, Map<String, Long> fileSizes) throws IOException {
    // Make a copy as we'll modify it.
    final Map<String,Long> filesToUpdate = new HashMap<>(fileSizes);
    ...
    if (!snapshotSizeChanges.isEmpty()) {
      if (LOG.isTraceEnabled()) {
        LOG.trace("Found " + filesToUpdate.size() +
            " files which were not referenced by any snapshots: " + filesToUpdate);
      }
      persistSnapshotSizeChanges(snapshotSizeChanges);
    }
    // We were removing files from this map as we found a snapshot that referenced it. Those files
    // remaining in this map were referenced by no snapshot (and would be deleted soon by HBase).
    ...
    try (Table quotaTable = conn.getTable(QuotaTableUtil.QUOTA_TABLE_NAME)) {
      ...
      synchronized (getLockForNamespace(tn.getNamespaceAsString())) {
        ...
        for (Result result : existingSnapshotSizes) {
          ...
          updates.add(QuotaTableUtil.createPutSnapshotSize(tn, snapshot, previousSize + size));
        }
        ...
        if (totalSizeChange != 0) {
          ...
          updates.add(QuotaTableUtil.createPutNamespaceSnapshotSize(
        ...
        if (!failures.isEmpty()) {
          throw new IOException("Failed to write some snapshot size updates: " + failures);
        }
      }
  ...
  @Override
  public Set<String> getRecentlyArchivedFiles() {
    return Collections.unmodifiableSet(recentlyArchivedFiles.keySet());
  }
  ...
  public long computeAndStoreSnapshotSizes(
    ...
    try {
      ...
      // clear the set as the recorded value in the quota table is accurate
      recentlyArchivedFiles.clear();
  ...
  void persistSnapshotSizes(
    ...
    table.put(snapshotSizes
        .stream()
        .map(sws -> QuotaTableUtil.createPutSnapshotSize(
