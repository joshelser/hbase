 ...
 *
 * This object needs to ensure that {@link #addArchivedFiles(Map)} and
...
public interface FileArchiverNotifier {
   ...
   *
   * @param fileSizes A collection of file name to file size in bytes
   * @throws IOException If there was an IO-related error persisting the file size(s)
   */
  void addArchivedFiles(Map<String, Long> fileSizes) throws IOException;

  /**
   * Returns the collection of files that were archived since the last time the size of a table
   * and its snapshosts was computed.
   *
   * @return Files archived since the last call to {@link #computeAndStoreSnapshotSizes(Collection)} 
   */
  Set<String> getRecentlyArchivedFiles();
