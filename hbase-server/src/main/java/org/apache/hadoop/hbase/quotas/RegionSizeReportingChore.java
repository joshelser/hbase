...
public class RegionSizeReportingChore extends ScheduledChore {
  ...
  public RegionSizeReportingChore(RegionServerServices rsServices) {
    super(RegionSizeReportingChore.class.getSimpleName(), rsServices, getPeriod(rsServices.getConfiguration()),
        getInitialDelay(rsServices.getConfiguration()), getTimeUnit(rsServices.getConfiguration()));
  ...
  void _chore() {
    ...
    HashSet<HRegionInfo> onlineRegionInfos = getOnlineRegionInfos(rsServices.getOnlineRegions());
  ...
  HashSet<HRegionInfo> getOnlineRegionInfos(List<Region> onlineRegions) {
    HashSet<HRegionInfo> regionInfos = new HashSet<>();
    onlineRegions.forEach((region) -> regionInfos.add(region.getRegionInfo()));
    return regionInfos;
  }

  void removeNonOnlineRegions(RegionSizeStore store, Set<HRegionInfo> onlineRegions) {
    ...
    Iterator<Entry<HRegionInfo,RegionSize>> iter = store.iterator();
    int numEntriesRemoved = 0;
    while (iter.hasNext()) {
      Entry<HRegionInfo,RegionSize> entry = iter.next();
